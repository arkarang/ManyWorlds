...
public abstract class BasicPacket implements WorldPacket{
    ...
    BasicPacket(ServerView from, ServerView to, WorldInform inform){
        this.from = from;
        this.to = to;
        this.inform = inform;
    }
    ...
    protected static void write(ByteBuf buf, WorldInform inform){
        ...
        ByteBufUtils.writeString(buf, inform.getType());
    ...
    protected static WorldInform read(ByteBuf buf){
        ...
        String type = ByteBufUtils.readString(buf);
        String name = ByteBufUtils.readString(buf);
        return new WorldInform(WorldToken.get(category), type, name);
    }
